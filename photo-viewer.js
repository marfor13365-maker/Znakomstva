...
//
// ФИКС ЭТОЙ ВЕРСИИ: формула зума считала сдвиг (translate) правильно только если точка
// трансформации (transform-origin) находится в (0,0) элемента — а по умолчанию у CSS
// transform-origin стоит в ЦЕНТРЕ элемента. Из-за этого при каждом изменении масштаба
// появлялась лишняя ошибка позиционирования, пропорциональная половине размера картинки —
// и фото "улетало" в угол при зуме. Фикс — одна строка: явно ставим transform-origin:0 0
// при инициализации, тогда формула становится математически точной.
//
// (сохранены также: pinch-to-zoom двумя пальцами, двойной тап для зума, панорамирование
// увеличенного фото одним пальцем, зум колёсиком на десктопе. reset() сбрасывает только
// визуальное состояние — НЕ снимает обработчики жестов, это отдельная detach().)
window.BlizkoPhotoViewer = (function() {
  ...
  function attach(el, options) {
    ...
    el.style.transformOrigin = '0 0'; // критично для корректной формулы зума ниже
    ...
    // Работает точно ТОЛЬКО при transform-origin:0 0 (см. комментарий в начале файла) —
    // при любом другом origin в формулу нужно добавлять поправку на origin*(newScale-oldScale),
    // которую мы сознательно избегаем, фиксируя origin в (0,0).
